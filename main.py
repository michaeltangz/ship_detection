...
filename_rgb = "True_color_1.png" # available at https://apps.sentinel-hub.com/eo-browser/?lat=-24.1176&lng=-46.3688&zoom=12&time=2019-04-02
filename_green = "B03_1.png" 
filename_nir = "B08_1.png" 

# load the datasets
dataset_rgb = gdal.Open(filename_rgb, gdalconst.GA_ReadOnly) 
dataset_green = gdal.Open(filename_green, gdalconst.GA_ReadOnly) 
dataset_nir = gdal.Open(filename_nir, gdalconst.GA_ReadOnly) 
...
for i in range(1,rows):
	for j in range(1,columns):
		...
		unique_values, counts = np.unique(values.ravel(), return_counts=True)
		mode_threshold_ndwi[i, j] = unique_values[np.argmax(counts)]
output_fig = plt.figure()
