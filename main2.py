...
threshold_for_ship = 220 # Adjusted for 0-255 scaled NDWI. Increased to reduce FPs.

# define filenames
filename_rgb = "true3.png" # available at https://apps.sentinel-hub.com/eo-browser/?lat=-24.1176&lng=-46.3688&zoom=12&time=2019-04-02
# Removed filenames for green and nir bands as we will load NDWI directly.
filename_ndwi = "ndwi3.png" # Assuming you have this file ready with NDWI values.
...
# Determine NDWI threshold using Otsu's method
...
filename_in_results = "results/tnOtsu_ts"+str(threshold_for_ship).zfill(3)+"_" # Updated filename for Otsu

# create plot with ndwi band with threshold
threshold_ndwi = array_ndwi > threshold_for_ndwi_otsu # Apply the Otsu threshold
output_fig = plt.figure()
...
eroded_mode_threshold_ndwi = ndi.binary_erosion(mode_threshold_ndwi, iterations=2).astype(mode_threshold_ndwi.dtype)
...
line_detection_mask_horizontal = np.array((-1, -1, -1, 2, 2, 2, -1, -1, -1)).reshape(3,3) # Reshape masks to 3x3
line_detection_mask_plus45 = np.array((-1, -1, 2, -1, 2, -1, 2, -1, -1)).reshape(3,3)
line_detection_mask_vertical = np.array((-1, 2, -1, -1, 2, -1, -1, 2, -1)).reshape(3,3)
line_detection_mask_minus45 = np.array((2, -1, -1, -1, 2, -1, -1, -1, 2)).reshape(3,3)
...
for i in range(1,rows - 1):
    for j in range(1,columns - 1):
        ...
        if eroded_mode_threshold_ndwi[i, j]:
            ...
            horizontal = np.abs((values * line_detection_mask_horizontal).sum()) # Use reshaped mask
            plus45 = np.abs((values * line_detection_mask_plus45).sum())
            vertical = np.abs((values * line_detection_mask_vertical).sum())
            minus45 = np.abs((values * line_detection_mask_minus45).sum())
...
# Animation code removed to save time.
